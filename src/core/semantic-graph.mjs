...
function stableEntries(entries) {
    return list(entries).slice().sort(byId);
}
 ...
 */
export function semanticGraph(value, { locale: requestedLocale } = {}) {
    ...
    const addEdge = (from, relation, to) => {
        const edge = { from, relation, to, origin: "pkl", evidenceStatus: "declared" };
    ...
    const addTerm = (termId) => {
        ...
        if (term)
            return addNode(`term/${term.id}`, "term", localized(term.text, locale) || `term ${term.id}`);
    ...
    const addRule = (ruleId) => {
        ...
        if (rule)
            return addNode(`rule/${rule.id}`, "rule", localized(rule.text, locale) || `rule ${rule.id}`);
        return addNode(`unresolved/rule/${escapedId(ruleId)}`, "unresolved-rule", `Unresolved rule ${ruleId ?? "missing"}`);
    };
    const addDecision = (decisionId) => {
        const decision = decisionById.get(String(decisionId));
        if (decision)
            return addNode(`decision/${decision.id}`, "decision", localized(decision.summary, locale) || `decision ${decision.id}`);
    ...
    for (const rule of stableEntries(model.rules)) {
        ...
        for (const termId of list(rule.terms).slice().sort())
            addEdge(ruleId, "uses-term", addTerm(termId));
        ...
        for (const implementation of list(rule.implementedBy).slice().sort((left, right) => artifactId(left).localeCompare(artifactId(right)))) {
    ...
    for (const decision of stableEntries(model.decisions)) {
        ...
        for (const superseded of list(decision.supersedes).slice().sort())
            addEdge(decisionId, "supersedes", addDecision(superseded));
    }
    ...
    if (Object.keys(domainModel).length > 0) {
        ...
        for (const node of list(graph.nodes))
            addNode(String(node.id), String(node.kind), String(node.label));
        for (const edge of list(graph.edges))
            addEdge(String(edge.from), String(edge.relation), String(edge.to));
        errors.push(...list(graph.errors));
    }
    ...
    const refinementById = new Map();
    for (const process of stableEntries(intent.processes)) {
        for (const refinement of stableEntries(process.refinements))
            refinementById.set(String(refinement.id), refinement);
    }
    const addCapability = (capabilityId) => {
        const capability = capabilityById.get(String(capabilityId));
        if (capability)
            return addNode(`intent/capability/${capability.id}`, "intent-capability", localized(capability.text, locale) || `capability ${capability.id}`);
    ...
    const addOutcome = (outcomeId) => {
        const outcome = outcomeById.get(String(outcomeId));
        if (outcome)
            return addNode(`intent/outcome/${outcome.id}`, "intent-outcome", localized(outcome.text, locale) || `outcome ${outcome.id}`);
    ...
    const addProcess = (processId) => {
        const process = processById.get(String(processId));
        if (process)
            return addNode(`intent/process/${process.id}`, "intent-process", localized(process.text, locale) || `process ${process.id}`);
    ...
    const addClaim = (claimId) => {
        const claim = claimById.get(String(claimId));
        if (claim)
            return addNode(`intent/claim/${claim.id}`, "intent-claim", localized(claim.text, locale) || `claim ${claim.id}`);
        return addNode(`unresolved/intent-claim/${escapedId(claimId)}`, "unresolved-intent-claim", `Unresolved claim ${claimId ?? "missing"}`);
    };
    const addRefinement = (refinementId) => {
        const refinement = refinementById.get(String(refinementId));
        if (refinement)
            return addNode(`intent/refinement/${refinement.id}`, "intent-refinement", localized(refinement.text, locale) || `refinement ${refinement.id}`);
    ...
    for (const outcome of stableEntries(intent.outcomes)) {
        ...
        if (outcome.state)
            addEdge(outcomeId, "has-state", addTerm(outcome.state));
    ...
    for (const process of stableEntries(intent.processes)) {
        ...
        if (process.input)
            addEdge(processId, "accepts-input", addTerm(process.input));
        for (const outcome of list(process.outcomes).slice().sort())
            addEdge(processId, "permits-outcome", addOutcome(outcome));
        for (const capability of list(process.requires).slice().sort())
            addEdge(processId, "requires-capability", addCapability(capability));
        for (const outcome of list(process.constructs).slice().sort())
            addEdge(processId, "constructs-outcome", addOutcome(outcome));
        for (const implementation of list(process.implementedBy).slice().sort((left, right) => artifactId(left).localeCompare(artifactId(right)))) {
    ...
    for (const goal of stableEntries(intent.goals)) {
        ...
        for (const process of list(goal.intents).slice().sort())
            addEdge(goalId, "includes-process", addProcess(process));
        for (const claim of list(goal.claims).slice().sort())
            addEdge(goalId, "includes-claim", addClaim(claim));
    }
    for (const claim of stableEntries(intent.claims)) {
        const claimId = addClaim(claim.id);
        addEdge(modelId, "declares-claim", claimId);
        for (const process of list(claim.processes).slice().sort())
            addEdge(claimId, "claims-process", addProcess(process));
    }
    for (const task of stableEntries(intent.assuranceTasks)) {
        ...
        for (const claim of list(task.claims).slice().sort())
            addEdge(taskId, "assures-claim", addClaim(claim));
    ...
    for (const binding of stableEntries(intent.semanticBindings)) {
        ...
        for (const claim of list(binding.claims).slice().sort())
            addEdge(bindingId, "binds-claim", addClaim(claim));
        addEdge(bindingId, "binds-process", addProcess(binding.process));
        if (binding.refinement)
            addEdge(bindingId, "binds-refinement", addRefinement(binding.refinement));
...
function externalEvidenceStatus(status) {
    return status === "pass" ? "verified" : "observed";
}
function sourceModel(document) {
    return record(document.model ?? record(document.manifest).model);
}
function sourceArtifacts(document) {
...
function externalNodeId(scope, id) {
 ...
 */
export function semanticGraphWithEvidence(graph, sources) {
    ...
    const addNode = (id, kind, label, origin, evidenceStatus) => {
        if (!nodes.has(id))
            nodes.set(id, { id, kind, label, origin, evidenceStatus });
        return id;
    };
    const addEdge = (from, relation, to, origin, evidenceStatus) => {
        edges.set(`${from}\u0000${relation}\u0000${to}`, { from, relation, to, origin, evidenceStatus });
    };
    const pklDeclaration = (path) => addNode(`pkl-declaration/${escapedId(path)}`, "pkl-declaration", `Pkl declaration ${path ?? "missing"}`, "pkl", "declared");
    if (sources.conformance) {
        ...
        for (const target of stableEntries(document.targets)) {
            ...
            if (nodes.has(ruleId))
                addEdge(ruleId, "has-conformance-result", targetId, "conformance-report", externalEvidenceStatus(target.status));
            else
                addEdge(targetId, "tests-unresolved-rule", addNode(`unresolved/rule/${escapedId(target.ruleId)}`, "unresolved-rule", `Unresolved rule ${target.ruleId ?? "missing"}`, "conformance-report", "observed"), "conformance-report", externalEvidenceStatus(target.status));
    ...
    if (sources.assurance) {
        ...
        for (const artifact of sourceArtifacts(document)) {
            ...
            for (const propertyId of list(artifact.propertyIds).slice().sort()) {
    ...
    if (sources.realApp) {
        ...
        for (const check of stableEntries(document.checks)) {
            const checkId = addNode(`evidence/real-app/${escapedId(check.kind)}/${escapedId(check.id)}`, "real-app-observation", `${check.kind ?? "observed"} ${check.id ?? "missing"}: ${check.status ?? "unknown"}`, "real-app-reconciliation", "observed");
...
function graphdbDocuments(graph, ids) {
...
function graphdbRelationStableId(edge) {
...
function graphdbRelationTags(edge) {
    return [
        "relation",
...
function graphdbRelationNodes(graph) {
...
function graphdbRelationDocument(relation, graph, ids) {
...
function hashVector(value, dimensions) {
...
function csvVector(vector) {
    return vector.map((entry) => {
        const rounded = Number(entry.toFixed(8));
        return Object.is(rounded, -0) ? "0.00000000" : rounded.toFixed(8);
    }).join(",");
}
function graphdbDocumentRecord(value) {
    const document = record(value);
    if (!/^\d+$/.test(String(document.graphdbId ?? "")))
        throw new Error(`invalid GraphDB document id: ${document.graphdbId ?? "missing"}`);
    if (typeof document.text !== "string")
        throw new Error(`invalid GraphDB document text: ${document.graphdbId}`);
    return {
        graphdbId: String(document.graphdbId),
        stableId: String(document.stableId ?? ""),
        title: String(document.title ?? ""),
        tags: list(document.tags).map(String),
        text: document.text,
    };
}
/**
 ...
 */
export function embedGraphdbDocuments(values, { dimensions = 256 } = {}) {
    if (!Number.isInteger(dimensions) || dimensions < 2 || dimensions > 8192)
    ...
    for (const document of documents) {
        if (ids.has(document.graphdbId))
            throw new Error(`duplicate GraphDB document id: ${document.graphdbId}`);
 ...
 */
export function querySemanticGraph(graph, question, { dimensions = 256, limit = 5, hops = 1 } = {}) {
    if (!Number.isInteger(limit) || limit < 1 || limit > 50)
        throw new Error(`invalid graph query limit: ${limit}`);
    if (!Number.isInteger(hops) || hops < 0 || hops > 3)
        throw new Error(`invalid graph query hops: ${hops}`);
    ...
    const matches = ranked.map(({ node, score }) => {
        ...
        const relations = graph.edges
            ...
            return { direction: outgoing ? "outgoing" : "incoming", relation: edge.relation, stableId: other, label: byId.get(other)?.label ?? other };
        })
    ...
    const evidence = [
        ...matches.map((match) => ({ ref: `node:${match.stableId}`, kind: "node", origin: match.origin, evidenceStatus: match.evidenceStatus })),
        ...matches.flatMap((match) => match.relations.map((relation) => {
            ...
            return edge ? { ref: `edge:${edge.from}#${edge.relation}->${edge.to}`, kind: "edge", origin: edge.origin, evidenceStatus: edge.evidenceStatus } : null;
...
export function renderSemanticGraphQueryMarkdown(report) {
    const lines = [
    ...
    if (report.matches.length === 0)
        lines.push("- No source-backed declaration matched this query.");
    for (const match of report.matches) {
        ...
        for (const relation of match.relations)
            lines.push(`- ${relation.direction}: \`${relation.relation}\` → \`${relation.stableId}\` (${relation.label})`);
        lines.push("");
    }
    lines.push("## Evidence", "");
    ...
    for (const entry of report.evidence)
        lines.push(`- \`${entry.ref}\` (${entry.origin}/${entry.evidenceStatus})`);
 ...
 */
export function graphdbBundle(graph) {
    ...
    for (const node of projectionNodes) {
        ...
        if (collision && collision !== node.id)
            throw new Error(`GraphDB id collision: ${collision} and ${node.id}`);
        ids.set(node.id, id);
    ...
    const graphdbEdges = [...new Set(graph.edges.flatMap((edge) => {
            ...
            if (!relation || !from || !to)
                throw new Error("GraphDB relation projection lost a node id");
            return [
                from + "," + relation,
                relation + "," + to,
            ];
