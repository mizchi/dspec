...
type IntentHttpRefinement = {
  method: string;
  path: string;
};

type IntentTransactionRefinement = {
  dbTransaction: string;
};

type IntentRefinement = Identified & {
  http?: IntentHttpRefinement | null;
  kind?: string | null;
  transaction?: IntentTransactionRefinement | null;
};
...
type IntentClaim = Identified & {
  processes?: Array<string | null>;
  requiredImplementationBinding?: boolean | null;
};

export type IntentSemanticBinding = Identified & {
...
export function intentSemanticBindings(intent: unknown): IntentSemanticBinding[] {
  return list<IntentSemanticBinding>(record(intent)?.semanticBindings);
}

export function validateIntentSemanticBindings(
  processesValue: unknown,
  claimsValue: unknown,
  cloudNodesValue: unknown,
  semanticBindingsValue: unknown,
): string[] {
  const errors: string[] = [];
  const processes = list<IntentProcess>(processesValue);
  const claims = list<IntentClaim>(claimsValue);
  const cloudNodes = list<Identified>(cloudNodesValue);
  const semanticBindings = list<IntentSemanticBinding>(semanticBindingsValue);
  ...
  const bindingCoveredClaimIds = new Set<string | null | undefined>();
  const semanticBindingKeys = new Set<string>();

  for (const binding of semanticBindings) {
    const bindingClaims = list<string | null>(binding.claims);
    ...
    for (const claimId of bindingClaims) {
      ...
      if (!claim) {
        ...
        if (!list<string | null>(claim.processes).includes(binding.process)) {
