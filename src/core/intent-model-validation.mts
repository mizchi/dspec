...
} from "./intent-access-policy-validation.mjs";
import {
  intentAssuranceTasks,
  intentClaims,
  intentGoals,
  validateIntentGoalClaimAssurance,
} from "./intent-assurance-validation.mjs";
import {
  intentConstructionAuthorities,
  validateIntentConstructionAuthorities,
} from "./intent-construction-authority-validation.mjs";
import {
  intentCapabilities,
  intentOutcomes,
  validateIntentOutcomes,
} from "./intent-outcome-validation.mjs";
...
} from "./intent-refinement-validation.mjs";
import {
  intentScenarios,
  validateIntentScenarios,
} from "./intent-scenario-validation.mjs";
import {
  intentSemanticBindings,
  validateIntentSemanticBindings,
} from "./intent-semantic-binding-validation.mjs";
...
export type IntentPattern = UnknownRecord;
...
export function intentPattern(model: unknown): IntentPattern | null {
  const patterns = record(record(model)?.patterns);
  const intent = record(patterns?.intent);
  return intent ? intent as IntentPattern : null;
}

export function validateIntentCatalogUniqueness(intentValue: unknown): string[] {
  const errors: string[] = [];
...
export function validateIntentModel(modelValue: unknown): string[] {
  const errors: string[] = [];
