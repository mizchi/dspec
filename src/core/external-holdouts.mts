...
export type ExternalHoldoutReportEntry = {
  holdout: {
    id: string;
    sourceRepository: string;
    sourceRevision: string;
    capturedOn?: string;
    estimatedAuthoringMinutes?: number;
    manualMappings?: unknown[];
    exclusions?: unknown[];
  };
  authoredIntent: {
    evaluationPath: string;
    facts: RealAppImportFact[];
  };
...
export type ExternalHoldoutMutationInput = {
  id: string;
  sourceRepository: string;
  sourceBeforeRevision: string;
  sourceAfterRevision: string;
  beforeAppRoot: string;
  afterAppRoot: string;
  expectedAddedFacts: RealAppImportFact[];
  expectedRemovedFacts: RealAppImportFact[];
  addedFacts: RealAppImportFact[];
  removedFacts: RealAppImportFact[];
};

export type ExternalHoldoutMutationReport = {
  id: string;
  sourceRepository: string;
  sourceBeforeRevision: string;
  sourceAfterRevision: string;
  beforeAppRoot: string;
  afterAppRoot: string;
  expectedAdded: RealAppImportFact[];
  expectedRemoved: RealAppImportFact[];
  added: RealAppImportFact[];
  removed: RealAppImportFact[];
  status: "pass" | "fail";
  errors: string[];
};

export type ExternalHoldoutCorpusInput = {
  id: string;
  holdouts: ExternalHoldoutReportEntry[];
  mutations: ExternalHoldoutMutationReport[];
};

function factKey(fact: RealAppImportFact): string {
  return `${fact.kind}:${fact.id}`;
}

export function normalizeRealAppImportFacts(facts: readonly RealAppImportFact[] = []): RealAppImportFact[] {
  return facts
    .map((fact) => ({ kind: fact.kind, id: fact.id }))
    .sort((left, right) => factKey(left).localeCompare(factKey(right)));
}

export function realAppImportFactListsEqual(
  left: readonly RealAppImportFact[] = [],
  right: readonly RealAppImportFact[] = [],
): boolean {
...
export function externalHoldoutMutationReport(
  input: ExternalHoldoutMutationInput,
...
export function externalHoldoutCorpusReport(input: ExternalHoldoutCorpusInput) {
...
export function renderExternalHoldoutCorpusMarkdown(report: ReturnType<typeof externalHoldoutCorpusReport>): string {
  const lines = [
