...
export function intentScenarios(intent) {
    return list(record(intent)?.scenarios);
}
export function validateIntentScenarios(vocabularyValue, processesValue, outcomesValue, scenariosValue) {
    const errors = [];
    const vocabulary = list(vocabularyValue);
    const processes = list(processesValue);
    const outcomes = list(outcomesValue);
    const scenarios = list(scenariosValue);
