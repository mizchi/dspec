...
export function validateClauseAst(ast, { context = "Clause.ast" } = {}) {
    ...
    const rejectName = () => {
        if (hasName)
            fail(`${node.op} does not accept name`);
    };
    const rejectArgs = () => {
        if (args.length > 0)
            fail(`${node.op} does not accept args`);
    };
    const rejectChildren = () => {
        if (children.length > 0)
            fail(`${node.op} does not accept children`);
    };
    if (node.op === "atom") {
        if (!hasName)
            fail("atom expects name");
        rejectChildren();
    }
        ...
        rejectChildren();
        if (args.length !== 2)
            fail(`${node.op} expects exactly 2 args`);
    }
    else if (node.op === "not") {
        rejectName();
        rejectArgs();
        if (children.length !== 1)
            fail("not expects exactly 1 child");
    }
    else if (node.op === "and" || node.op === "or") {
        rejectName();
        rejectArgs();
        if (children.length === 0)
            fail(`${node.op} expects at least 1 child`);
    }
    else if (node.op === "implies") {
        rejectName();
        rejectArgs();
        if (children.length !== 2)
            fail("implies expects exactly 2 children");
    }
    else if (node.op === "exists" || node.op === "forall") {
        if (!hasName)
            fail(`${node.op} expects bound variable name`);
        rejectArgs();
        if (children.length !== 1)
            fail(`${node.op} expects exactly 1 child`);
    }
    else {
...
export function evaluateClauseAst(ast, interpretation, bindings = {}, { version = CLAUSE_AST_SEMANTICS_VERSION } = {}) {
    ...
    const evaluate = (node, environment) => {
        ...
        if (node.op === "atom")
            return Boolean(interpretation.atom(node.name, args, environment));
        if (node.op === "eq")
            return Object.is(args[0], args[1]);
        if (node.op === "neq")
            return !Object.is(args[0], args[1]);
        if (node.op === "not")
            return !evaluate(children[0], environment);
        if (node.op === "and")
            return children.every((child) => evaluate(child, environment));
        if (node.op === "or")
            return children.some((child) => evaluate(child, environment));
        if (node.op === "implies") {
            return !evaluate(children[0], environment) || evaluate(children[1], environment);
        }
        const domain = list(interpretation.domain);
        const quantified = (value) => evaluate(children[0], { ...environment, [String(node.name)]: value });
        if (node.op === "exists")
            return domain.some(quantified);
        if (node.op === "forall")
            return domain.every(quantified);
        return false;
    };
