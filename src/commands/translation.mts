...
import type { TranslationCheckReport } from "../core/translation-lock.mjs";
import { CommandError } from "./error.mjs";

type TranslationOperation = "check" | "reconcile";
...
type TranslationReport = TranslationCheckReport;

type TranslationCommandContext = {
...
export function translationUsage(): string {
  return `usage:
...
export function parseTranslationArgs(args: readonly string[]): TranslationOptions {
  const [operationValue, ...rest] = args;
  if (operationValue !== "reconcile" && operationValue !== "check") {
    throw new CommandError(translationUsage());
  }
  const operation: TranslationOperation = operationValue;
  let json = false;
  let gate = false;
...
export function defaultTranslationLockPath(modelFile: string): string {
  const extension = ".pkl";
...
export function renderTranslationReport(report: TranslationReport): string {
...
export function runTranslationCommand(
  args: readonly string[],
  context: TranslationCommandContext,
): void {
  if (hasHelpFlag(args)) {
  ...
  if (options.operation === "reconcile") {
    ...
    context.write(
      `ok: reconciled translations ${report.lock.path} (${report.lock.bindings} bindings)\n`,
    );
    return;
  }
  ...
  if (!existsSync(lockPath)) {
    throw new CommandError(
      `translation lock not found: ${lockPath}; run dspec translation reconcile ${options.file}\n`,
    );
  }
  const report: TranslationReport = translationCheck(
    document,
    context.readJsonFile(lockPath, "translation lock"),
  );
  if (options.json) context.write(context.stableJson(report));
  else context.write(renderTranslationReport(report));
