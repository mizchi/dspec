...
export function dailyDriftUsage(): string {
  return `usage:
...
export function runDailyDriftCommand(args: readonly string[], { write, writeError, setExitCode }: DailyDriftContext): void {
  ...
  if (typeof result.status === "number" && result.status !== 0) setExitCode(result.status);
}
