...
import { pklImportPath, resolveSchemaModulePath, schemaImportFromModel, } from "./schema-lock.mjs";
...
export function parseScaffoldRuleReference(value, option) {
...
export function parseScaffoldRuleArgs(args) {
    let json = false;
    let force = false;
    let outputFile = null;
    let kind = "invariant";
...
export function scaffoldRuleDocument({ modelFile, outputFile = null, ruleId, kind, terms, implementation, test, }, context) {
    const model = context.loadModel(modelFile);
    const modelObject = record(model);
    const vocabulary = new Set(list(modelObject?.vocabulary)
    ...
    if (implementation) {
        lines.push("  implementedBy {", `    d.codeRef(${pklString(implementation.path)}, ${pklString(implementation.anchor)})`, "  }");
    }
    lines.push("}");
    return {
...
export function writeScaffoldedRule(outputFile, source, force = false) {
...
export function runScaffoldCommand(args, context) {
