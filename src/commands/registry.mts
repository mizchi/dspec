export type TopLevelCommand = Readonly<{
  name: string;
  usage: string;
}>;
...
export function topLevelCommand(name: string): TopLevelCommand | undefined {
...
export function topLevelCommandHelp(command: TopLevelCommand): string {
  return `usage:\n  ${command.usage}\n`;
}
