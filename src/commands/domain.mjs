...
import { domainCodegenIr, domainRelationshipGraph, renderDomainRelationshipMarkdown, renderDomainRelationshipMermaid, renderDomainTypescript, } from "../core/domain.mjs";
...
export function parseDomainArgs(args, subcommand, { fail = (message) => { throw new Error(message); } } = {}) {
    let json = false;
    let markdown = false;
    let mermaid = false;
    let language = null;
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--language") {
            ...
            if (!language || language.startsWith("-"))
                fail(`domain generate requires a language\n${domainUsage()}`);
            continue;
        }
        if (arg === "--output") {
            ...
            if (!outputFile || outputFile.startsWith("-"))
                fail(`domain generate requires an output path\n${domainUsage()}`);
            continue;
        }
        if (arg.startsWith("-"))
            fail(`unknown domain ${subcommand} option: ${arg}\n${domainUsage()}`);
        positional.push(arg);
    }
    if (positional.length !== 1)
        fail(domainUsage());
    if (Number(json) + Number(markdown) + Number(mermaid) > 1)
        fail(`domain ${subcommand} accepts one output format\n${domainUsage()}`);
    if (subcommand === "ir" && (language || outputFile || markdown || mermaid))
        fail(`domain ir does not accept --language, --output, --markdown, or --mermaid\n${domainUsage()}`);
    ...
    if (subcommand === "generate" && (markdown || mermaid))
        fail(`domain generate does not accept --markdown or --mermaid\n${domainUsage()}`);
    if (subcommand === "relationships" && language)
        fail(`domain relationships does not accept --language\n${domainUsage()}`);
    return { json, markdown, mermaid, language, outputFile, modelFile: positional[0] };
}
function writeGeneratedSource(path, source) {
    mkdirSync(dirname(resolve(path)), { recursive: true });
    writeFileSync(path, source);
    return { path, bytes: Buffer.byteLength(source, "utf8"), digest: assuranceDigest(source) };
}
export function runDomainCommand(args, { fail, loadModel, stableJson, validate, write }) {
    ...
    if (subcommand === "relationships") {
        ...
        if (options.json && output)
            write(stableJson({ ...report, output }));
        else if (output)
            write(`ok: ${model.id} generated domain relationship document ${output.path}\n`);
        else
            write(source);
        if (report.status === "fail")
            fail("domain relationship generation failed\n");
        return;
    }
    ...
    if (subcommand === "ir") {
        ...
        if (report.status === "fail")
            fail("domain IR generation failed\n");
        return;
    }
    if (report.status === "fail") {
        if (options.json)
