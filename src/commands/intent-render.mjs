...
export function renderIntentMutationMarkdown(report) {
    const lines = [
...
export function renderIntentTraceMarkdown(report) {
    const lines = [
    ...
    if (report.executionPolicy) {
        ...
        if (report.executionPolicy.reason)
            lines.push(`- reason: ${report.executionPolicy.reason}`);
    ...
    for (const assumption of report.evidence.assumptions)
        lines.push(`- ${assumption}`);
    return `${lines.join("\n")}\n`;
}
export function renderIntentCoverageMarkdown(report) {
    const lines = [
...
export function renderIntentScenarioCorpusMarkdown(report) {
    const lines = [
