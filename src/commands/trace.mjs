...
import { createTraceLock, traceCheck, traceSnapshot, } from "../core/trace-lock.mjs";
...
export function defaultTraceLockPath(modelFile) {
    const extension = ".pkl";
...
export function scopeTraceReportToChangedPaths(report, modelPath, changedPaths) {
    const drift = report.drift.filter((entry) => {
        if (entry.kind.startsWith("rule-"))
            return changedPaths.has(modelPath);
...
function scopeTraceReportToDiff(report, modelFile) {
    ...
    return scopeTraceReportToChangedPaths(report, relativeWorktreePath(modelFile), changed.paths);
}
...
export function runTraceCommand(args, context) {
    if (hasHelpFlag(args)) {
    ...
    let report = traceCheck(document, context.readJsonFile(lockPath, "trace lock"));
    if (options.diff)
        report = scopeTraceReportToDiff(report, options.file);
    else
        report = { ...report, scope: { kind: "all", changedPaths: [] } };
    if (options.json)
        context.write(context.stableJson(report));
    else
        context.write(renderTraceReport(report));
