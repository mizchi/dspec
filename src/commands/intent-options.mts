...
export type IntentTraceArgs = Omit<OutputFormat, "positional"> & {
  timeoutMs: number;
...
export type IntentProtocolTestArgs = Omit<OutputFormat, "positional"> & {
  timeoutMs: number;
  httpBaseUrl: string | null;
  grpcRunner: string | null;
  modelFile: string;
};
...
function parseOutputOptions(args: readonly string[], subcommand: string, supportedOptions: ReadonlySet<string>): OutputFormat {
  let json = false;
  let markdown = false;
  let outputFile = null;
  const positional = [];
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--output" && supportedOptions.has(arg)) {
      ...
      index += 1;
      if (!outputFile) optionError("missing value for --output");
      continue;
    }
    if (arg.startsWith("-")) optionError(`unknown intent ${subcommand} option: ${arg}`);
    positional.push(arg);
  }
  if (json && markdown) optionError(`intent ${subcommand} accepts only one output format`);
  return { json, markdown, outputFile, positional };
}

export function parseIntentTraceArgs(args: readonly string[], subcommand: string): IntentTraceArgs {
  let json = false;
  let markdown = false;
  let outputFile = null;
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--markdown") { markdown = true; continue; }
    if (arg === "--output") {
      outputFile = args[index + 1] ?? null;
      index += 1;
      if (!outputFile) optionError("missing value for --output");
    ...
    if (arg === "--timeout-ms") {
      ...
      index += 1;
      if (subcommand !== "exercise" || !value || !/^[1-9][0-9]*$/.test(value)) optionError("--timeout-ms requires a positive integer for intent exercise");
      timeoutMs = Number(value);
      continue;
    }
    if (arg === "--policy") {
      if (subcommand !== "exercise") optionError("--policy is available only for intent exercise");
      policy = true;
      continue;
    }
    if (arg === "--http-base-url") {
      const value = args[index + 1] ?? null;
      index += 1;
      if (subcommand !== "exercise" || !value) optionError("--http-base-url requires a URL for intent exercise");
      try {
...
export function parseIntentProtocolTestArgs(args: readonly string[], subcommand: string): IntentProtocolTestArgs {
  let json = false;
  let markdown = false;
  let outputFile = null;
  let timeoutMs = DEFAULT_INTENT_EXERCISE_TIMEOUT_MS;
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--timeout-ms") {
      const value = args[index + 1] ?? null;
      index += 1;
      if (subcommand !== "test" || !value || !/^[1-9][0-9]*$/.test(value)) optionError("--timeout-ms requires a positive integer for intent test");
      timeoutMs = Number(value);
      continue;
    }
    if (arg === "--http-base-url") {
      const value = args[index + 1] ?? null;
      index += 1;
      if (subcommand !== "test" || !value) optionError("--http-base-url requires a URL for intent test");
      try {
        const url = new URL(value);
        if (!["http:", "https:"].includes(url.protocol)) throw new Error("unsupported protocol");
      } catch {
    ...
    if (arg === "--grpc-runner") {
      ...
      index += 1;
      if (subcommand !== "test" || !grpcRunner || grpcRunner.startsWith("-")) optionError("--grpc-runner requires a script or executable for intent test");
      continue;
    }
    if (arg.startsWith("-")) optionError(`unknown intent ${subcommand} option: ${arg}`);
    positional.push(arg);
  ...
  if (json && markdown) optionError(`intent ${subcommand} accepts only one output format`);
  if (subcommand === "generate-tests" && markdown) optionError("intent generate-tests supports JSON only");
...
export function parseIntentAccessArgs(args: readonly string[]): { json: boolean; modelFile: string; process: string; subject: string } {
...
export function parseIntentBindingArgs(args: readonly string[]): { json: boolean; markdown: boolean; modelFile: string; observedFile: string } {
...
export function parseIntentGraphArgs(args: readonly string[]): { json: boolean; markdown: boolean; locale: string | null; modelFile: string } {
  let json = false;
  let markdown = false;
  let locale = null;
  const positional = [];
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--locale") {
      locale = args[index + 1];
      index += 1;
      if (!locale || locale.startsWith("-")) optionError("intent graph requires a locale value");
      continue;
    }
    if (arg.startsWith("-")) optionError(`unknown intent graph option: ${arg}`);
    positional.push(arg);
  }
  if (json && markdown) optionError("intent graph accepts only one output format");
  if (positional.length !== 1) throw new CommandError(intentUsage());
