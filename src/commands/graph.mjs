...
import { embedGraphdbDocuments, graphdbBundle, querySemanticGraph, renderSemanticGraphQueryMarkdown, renderSemanticGraphTurtle, semanticGraph, semanticGraphWithEvidence, } from "../core/semantic-graph.mjs";
export function graphUsage() {
    return `usage:
...
export function parseGraphArgs(args, { fail = (message) => { throw new Error(message); } } = {}) {
    let format = "json";
    let locale = null;
    let output = null;
    const positional = [];
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--format") {
            ...
            index += 1;
            if (value !== "json" && value !== "turtle" && value !== "graphdb")
                fail(`unknown graph format: ${value || "missing"}\n${graphUsage()}`);
            format = value;
            continue;
        }
        if (arg === "--locale") {
            ...
            if (!locale || locale.startsWith("-"))
                fail(`graph export requires a locale\n${graphUsage()}`);
            continue;
        }
        if (arg === "--output") {
            ...
            if (!output || output.startsWith("-"))
                fail(`graph export requires an output path\n${graphUsage()}`);
            continue;
        }
        if (arg.startsWith("-"))
            fail(`unknown graph export option: ${arg}\n${graphUsage()}`);
        positional.push(arg);
    }
    if (positional.length !== 1)
        fail(graphUsage());
    if (format === "graphdb" && !output)
        fail(`graphdb export requires --output <directory>\n${graphUsage()}`);
...
function writeGraphdbBundle(directory, graph, stableJson) {
...
function parsePositiveInteger(value, label, fail, minimum = 1, maximum = 8192) {
    const parsed = Number(value);
    if (!Number.isInteger(parsed) || parsed < minimum || parsed > maximum)
        fail(`invalid ${label}: ${value ?? "missing"}\n${graphUsage()}`);
    return parsed;
}
function parseRatio(value, label, fail) {
    const parsed = Number(value);
    if (!Number.isFinite(parsed) || parsed < 0 || parsed > 1)
        fail(`invalid ${label}: ${value ?? "missing"}\n${graphUsage()}`);
    return parsed;
}
function parseGraphEvidenceOptions(args, fail) {
    const output = [];
    const evidence = {};
    const flags = {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!path || path.startsWith("-"))
            fail(`${flag} requires a JSON file\n${graphUsage()}`);
        try {
...
function parseGraphEmbedArgs(args, fail) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg.startsWith("-"))
            fail(`unknown graph embed option: ${arg}\n${graphUsage()}`);
        positional.push(arg);
    }
    if (positional.length !== 1)
        fail(graphUsage());
...
function readJsonLines(path, fail) {
    try {
...
function parseGraphBuildArgs(args, fail) {
    let meandb = "meandb";
    let output = null;
    let metric = "cosine";
    let k = 8;
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--metric") {
            const value = args[index + 1];
            index += 1;
            if (value !== "cosine" && value !== "dot" && value !== "l2")
                fail(`invalid GraphDB metric: ${value ?? "missing"}\n${graphUsage()}`);
            metric = value;
            continue;
        }
        if (arg === "--k") {
        ...
        if (arg.startsWith("-"))
            fail(`unknown graph build option: ${arg}\n${graphUsage()}`);
        positional.push(arg);
    }
    if (positional.length !== 1)
        fail(graphUsage());
...
export function parseMeandbQueryArgs(args, { fail = (message) => { throw new Error(message); } } = {}) {
    let meandb = "meandb";
    let explain = false;
    const positional = [];
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg.startsWith("-"))
            fail(`unknown GraphDB query option: ${arg}\n${graphUsage()}`);
        positional.push(arg);
    }
...
function parseGraphQueryArgs(args, fail) {
    let locale = null;
    let dimensions = 256;
    let limit = 5;
    let hops = 1;
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--locale") {
            ...
            index += 1;
            if (!locale || locale.startsWith("-"))
                fail(`graph query requires a locale\n${graphUsage()}`);
            continue;
        }
        ...
        if (arg.startsWith("-"))
            fail(`unknown graph query option: ${arg}\n${graphUsage()}`);
        positional.push(arg);
    }
    if (json && markdown || positional.length < 2)
        fail(graphUsage());
    return { modelFile: positional[0], question: positional.slice(1).join(" "), locale, dimensions, limit, hops, json, markdown };
}
export function runGraphCommand(args, { fail, loadModel, stableJson, validate, write }) {
    ...
    if (subcommand === "build") {
        ...
        for (const filename of ["notes.csv", "links.csv", "meta.tsv"]) {
            if (!existsSync(join(options.directory, filename)))
                fail(`GraphDB bundle is missing ${filename}: ${options.directory}\n`);
        }
        ...
        if (options.minWeight !== null)
            argv.push("--min-weight", String(options.minWeight));
        ...
        if (result.error)
            fail(`failed to run meandb command ${options.meandb}: ${result.error.message}\n`);
        if (result.status !== 0)
            fail(`meandb build failed (${result.status ?? "unknown"}): ${String(result.stderr ?? result.stdout ?? "").trim()}\n`);
        write(stableJson(report));
        return;
    }
    if (subcommand === "query-dsl") {
        ...
        if (!existsSync(graph))
            fail(`GraphDB database does not exist: ${graph}\n`);
        if (!existsSync(query))
            fail(`GraphDB query file does not exist: ${query}\n`);
        ...
        if (result.error)
            fail(`failed to run meandb command ${options.meandb}: ${result.error.message}\n`);
        if (result.status !== 0)
            fail(`meandb query failed (${result.status ?? "unknown"}): ${String(result.stderr ?? result.stdout ?? "").trim()}\n`);
    ...
    if (subcommand === "export") {
        ...
        if (options.format === "graphdb") {
            ...
            if (options.output)
                write(stableJson({ status: report.status, model: report.model, format: options.format, output: writeFile(resolve(options.output), source) }));
            else
                write(source);
        }
        if (report.status === "fail")
            fail("semantic graph export failed\n");
        return;
    }
    if (subcommand === "query") {
        ...
        if (graph.status === "fail")
            fail(`semantic graph query failed\n${graph.errors.join("\n")}\n`);
        ...
        if (options.json)
            ...
            write(renderSemanticGraphQueryMarkdown(report));
        return;
    }
