...
import { compileAlloyBehaviorModel, validateAlloyBehaviorModel } from "../src/core/alloy-behavior.mjs";
...
if (!modelPath || !outputPath || values.length !== 2) {
  process.stderr.write("usage: node scripts/generate-alloy-behavior.mjs [--check] <model.pkl> <output.als>\n");
  process.exitCode = 64;
} else {
  ...
  if (errors.length > 0) {
    ...
    const source = compileAlloyBehaviorModel(document).alloySource;
    if (check) {
      if (!existsSync(output) || readFileSync(output, "utf8") !== source) {
        process.stderr.write(`generated Alloy behavior source drift: ${outputPath}\n`);
        process.exitCode = 1;
      }
    } else {
