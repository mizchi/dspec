...
import { compileTetrisAlloyModel, validateTetrisAlloyModel } from "../src/core/tetris-alloy.mjs";
...
if (!modelPath || !outputPath || values.length !== 2) {
  process.stderr.write("usage: node scripts/generate-tetris-alloy.mjs [--check] <model.pkl> <output.als>\n");
  process.exitCode = 64;
} else {
  ...
  if (errors.length > 0) {
    ...
    const source = compileTetrisAlloyModel(document).alloySource;
    if (check) {
      if (!existsSync(output) || readFileSync(output, "utf8") !== source) {
        process.stderr.write(`generated Tetris Alloy source drift: ${outputPath}\n`);
        process.exitCode = 1;
      }
    } else {
