...
- `tetris.active-piece` (state): Active piece
- `tetris.board` (entity): Board
- `tetris.cell` (value): Cell
- `tetris.game-over` (state): Game over
- `tetris.gravity` (action): Gravity step
- `tetris.line-clear` (event): Line clear
- `tetris.lock` (event): Piece lock
- `tetris.tetromino` (entity): Tetromino

## Rules

### TETRIS-ADVANCED-RULES-OUT-OF-SCOPE

SRS, hold, T-spin, and scoring are outside this specification
...
Every locked cell and active-piece cell lies within the 10 by 20 board
...
After a piece locks, each completely filled row is cleared and locked cells above it fall by that many rows
...
A piece that cannot descend under gravity is locked
...
A rotation that leaves the board or collides with a locked cell is rejected without changing state
...
A translation that leaves the board or collides with a locked cell is rejected without changing state
...
Active-piece cells neither overlap locked cells nor duplicate one another
...
The game enters game over when a new piece cannot spawn without overlapping a locked cell
...
When the spawn location is clear, starting a game creates exactly one active piece
...
After game over, movement, rotation, gravity, and locking are not accepted
