...
one sig R_DSPEC_GENERATED_QUINT_VERIFY extends ActiveApprovedRule {}
...
fact GeneratedRuleClasses {
  ActiveApprovedRule = R_DSPEC_APP_PROFILE + R_DSPEC_ASSURANCE_EVIDENCE_MANIFEST + R_DSPEC_AUTHORING_SHORTHAND + R_DSPEC_BACKEND_PROJECTION_OWNERSHIP + R_DSPEC_BACKEND_REPORT_COMPAT_FIXTURES + R_DSPEC_CHECK_APPROVED_VERIFIED + R_DSPEC_CHECK_ASSURANCE + R_DSPEC_CHECK_CONTRADICTION + R_DSPEC_CHECK_DRIFT_COVERAGE_JSON + R_DSPEC_CHECK_DUPLICATES + R_DSPEC_CHECK_REFERENCES + R_DSPEC_CLOUD_TOPOLOGY_PATTERN + R_DSPEC_COUNTEREXAMPLE_NORMALIZED + R_DSPEC_COVERAGE_APPROVED_CHECKED + R_DSPEC_COVERAGE_CLAUSE_QUALITY + R_DSPEC_DAILY_DRIFT_REVIEW + R_DSPEC_DATA_GOVERNANCE_PATTERN + R_DSPEC_DB_MIGRATION_MAPPING_COVERAGE + R_DSPEC_DB_MIGRATION_MAPPING_WELL_FORMED + R_DSPEC_DB_MIGRATION_PATTERN + R_DSPEC_DB_MODEL_PATTERN + R_DSPEC_DB_SCHEMA_IMPORTER + R_DSPEC_DOCUMENTED_CLI_EXAMPLES + R_DSPEC_DOGFOOD_TASK + R_DSPEC_DOMAIN_COVERAGE_ORACLE + R_DSPEC_DOMAIN_MODEL_FORMALIZATION_AND_CODEGEN + R_DSPEC_DOMAIN_PRESET_PACK + R_DSPEC_DRIFT_CHECK_TARGET + R_DSPEC_DRIFT_IMPLEMENTATION_REF + R_DSPEC_EMIT_FORMAL_BACKENDS + R_DSPEC_EMIT_MARKDOWN + R_DSPEC_EMIT_QUICKCHECK + R_DSPEC_EXPR_AST_PROJECTION + R_DSPEC_EXPR_OPAQUE + R_DSPEC_EXPR_TYPED_AST + R_DSPEC_FORMAL_SOURCE_OF_TRUTH + R_DSPEC_GENERATED_ALLOY_SYNTAX + R_DSPEC_GENERATED_ARTIFACT_FRESHNESS + R_DSPEC_GENERATED_CHECKS_LOAD_BEARING + R_DSPEC_GENERATED_LEAN_COMPILES + R_DSPEC_GENERATED_QUICKCHECK_RUNS + R_DSPEC_GENERATED_QUINT_VERIFY + R_DSPEC_I18N_RENDER + R_DSPEC_I18N_SEMANTIC_DRIFT + R_DSPEC_IMPLEMENTATION_CONFORMANCE + R_DSPEC_INTENT_CLOSED_CONSTRUCTION + R_DSPEC_INTENT_GOAL_GRAPH + R_DSPEC_INTENT_PROTOCOL_TEST_ORACLE + R_DSPEC_JSON_REPORT_COMPAT_FIXTURES + R_DSPEC_LEAN_EQ_SEMANTIC + R_DSPEC_MARKDOWN_REVIEW_ARTIFACT + R_DSPEC_NIX_CI_GATE + R_DSPEC_NIX_FORMAL_TOOLS + R_DSPEC_PACKAGE_RELEASE + R_DSPEC_PRODUCT_POSITIONING + R_DSPEC_REAL_APP_DOGFOOD + R_DSPEC_REAL_APP_IMPORTER + R_DSPEC_REAL_APP_RECONCILIATION + R_DSPEC_REAL_APP_REVERSE_COVERAGE + R_DSPEC_RELEASE_SAFETY_PATTERN + R_DSPEC_RUNTIME_COLLECTOR_FIXTURE + R_DSPEC_RUNTIME_COLLECTOR_MANIFEST + R_DSPEC_RUNTIME_EVIDENCE_COLLECTOR + R_DSPEC_RUNTIME_EVIDENCE_IMPORTER + R_DSPEC_RUNTIME_EVIDENCE_PATTERN + R_DSPEC_RUNTIME_EVIDENCE_VERIFIER + R_DSPEC_RUNTIME_SAFETY_PATTERN + R_DSPEC_SCHEMA_TYPED + R_DSPEC_SOURCE_MAP_GENERATED + R_DSPEC_SPEC_CHANGE_REVIEW + R_DSPEC_SPEC_CHANGE_REVIEW_SCAFFOLD + R_DSPEC_SPEC_COMPAT_CLASSIFIER + R_DSPEC_SPEC_DIFF_IMPACT + R_DSPEC_SPEC_QUERY + R_DSPEC_SPEC_READING_EVAL + R_DSPEC_SQL_QUERY_ORACLE + R_DSPEC_STABLE_IDS + R_DSPEC_TOPLEVEL_MODEL + R_DSPEC_VERIFY_GENERATED_JSON
  DeprecatedRule = none
}
